...
object RunLSA {
  def main(args: Array[String]): Unit = {
    ...
    val docTexts: Dataset[(String, String)] = parseWikipediaDump("hdfs://localhost:9000/aas/ch06/ds/wikidump.xml")
    ...
    queryEngine.printTopTermsForTerm(termIds(0))
    queryEngine.printTopTermsForTerm(termIds(1))
    queryEngine.printTopTermsForTerm(termIds(2))

    queryEngine.printTopDocsForTerm(termIds(0))
    queryEngine.printTopDocsForTerm(termIds(1))

    queryEngine.printTopDocsForDoc(docIds(0))
    queryEngine.printTopDocsForDoc(docIds(1))
    queryEngine.printTopDocsForDoc(docIds(2))

    queryEngine.printTopDocsForTermQuery(Seq(termIds(0), termIds(1)))
  }
